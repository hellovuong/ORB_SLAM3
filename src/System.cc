    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    ...
    mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR || mSensor == ODOM_MONOCULAR || mSensor == ODOM_IMU_MONOCULAR, mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor == ODOM_IMU_MONOCULAR, mSensor == ODOM_MONOCULAR || mSensor == ODOM_IMU_MONOCULAR, strSequence);
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas,const g2o::SE2 &odo, string filename)
{
    if(mSensor!=MONOCULAR && mSensor!=IMU_MONOCULAR && mSensor!=ODOM_IMU_MONOCULAR)
    {
        cerr << "ERROR: you called TrackMonocular but input sensor was not set to Monocular nor Monocular-Inertial nor Monocular-Inertial-Odom." << endl;
        exit(-1);
    }
...
void System::SaveTrajectoryTUM_ns(const string &filename)
{
    cout << endl << "Saving camera trajectory to " << filename << " ..." << endl;
    if(mSensor==MONOCULAR)
    {
