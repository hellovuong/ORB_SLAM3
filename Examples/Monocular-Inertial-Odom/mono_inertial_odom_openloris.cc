...
int main(int argc, char **argv)
{
    ...
    cout << "Loading Odometry for sequence " << seq << "...";
    LoadOdom(string(argv[3]), vTimestampsOdom[seq], vOdo[seq]);
    cout << "LOADED!" << endl;
    ...
    if((nImages[seq]<=0)||(nImu[seq]<=0) || (nOdo[seq] <=0))
    {
        cerr << "ERROR: Failed to load images or IMU or Odometry for sequence" << seq << endl;
        return 1;
    }
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        vector<g2o::SE2> vOdoMeas = vOdo[seq];
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            SLAM.TrackMonocular(im,tframe,vImuMeas, vOdoMeas[ni]); // TODO change to monocular_inertial_odom
...
void LoadOdom(const string &strOdomPath, vector<double> &vTimeStamps, vector<g2o::SE2> &vOdo)
{
    ...
    //cout << strOdomPath + "/odom_interp.txt" << endl;
    vOdo.reserve(5000);
    double ts;
    Eigen::Vector3d xyr;
